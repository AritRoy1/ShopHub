...
from customer.models import Customer, Vendor, MultipleAddress
...
def VendorRegistration(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            img = form.cleaned_data['gst_invoice']
            ...
            vendor = Vendor(username=username, first_name=first_name, last_name=last_name,email=email, aadhar_number=aadhar_number, ac_number=ac_number, gst_invoice=img, address=address, phone_number=phone_number, city=city, state=state, zip_code=zip_code, birth_date=birth_date)
            logout(request)
            vendor.save()
            login(request, vendor)        
            # print(username, aadhar_number, ac_number, first_name, last_name, email)   
            print("lkj")
...
def deliveryAddress(request, prod_id):
    print("asd",request.user)
    print(prod_id)
    image = Image.objects.get(product__id=prod_id)
    stripe_publishable_key = settings.STRIPE_PUBLISHABLE_KEY

    print(image)
    multi = MultipleAddress.objects.get(customer__username = request.user)
    context = {
        "image":image,
        "stripe_publishable_key":stripe_publishable_key,
        "multi":multi     
    ...
    # return render(request, 'product/delivery_address.html',context)
